...
## Quick start (try it in ~2 minutes, no real data needed)

Once the requirements above are installed, this generates synthetic test
reads and runs the full pipeline against them — useful for confirming your
environment is set up correctly before pointing it at real data:

```bash
python3 test_data/make_test_data.py             # writes test_data/*.fastq.gz + reference
snakemake -c4 --configfile config/config.yaml -n   # dry run — check the plan
snakemake -c4 --configfile config/config.yaml       # actual run
cat results/summary.tsv                              # see per-sample stats
```

Expect it to finish in well under a minute and produce a `results/summary.tsv`
matching the numbers in [`docs/example_output/summary.tsv`](docs/example_output/summary.tsv)
...
`results/` itself is gitignored (it's pipeline output, regenerated by running
the commands above) — it isn't committed to the repo.

## Example output

A lightweight example of what `results/summary.tsv` looks like, from a run
against the bundled synthetic test data, is checked in at
[`docs/example_output/summary.tsv`](docs/example_output/summary.tsv) /
[`summary.html`](docs/example_output/summary.html):

| sample | reads_before_trim | reads_after_trim | contaminant_reads_removed | final_clean_reads |
|---|---|---|---|---|
| sample1 | 1100 | 1054 | 200 | 427 |
| sample2 | 1120 | 1068 | 300 | 384 |

(Full column set — quality rates, adapter-trimmed counts, etc. — is in the
linked TSV/HTML.) The full per-sample fastp HTML reports, BAMs, and FASTQs
from a test run aren't committed here (they're large, regenerable, and would
otherwise dominate the repo's language stats) — a fresh run's full `results/`
is uploaded as a downloadable artifact by the CI workflow on every push (see
the Actions tab), or you can just run the Quick start commands above locally.
